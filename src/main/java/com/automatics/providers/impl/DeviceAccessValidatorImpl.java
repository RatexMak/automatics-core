...
/**
 * 
 * Default implementation of device access validator. If required partner can
 * override this class in
 * partner-applicationContext.xml by mapping the custom implementation class
 * against bean name 'deviceAccessValidator'.
 *
 */
public class DeviceAccessValidatorImpl implements DeviceAccessValidator {
	...
	/**
	 * Verify if accessibility check of device is required or not by parsing
	 * device_config.json
	 */
	@Override
	...
	private boolean waitForDeviceIpAcquisition(Dut dut) {
		...
		try {
			...
			// 1. First Delay - For the device to be up. Wait for 1/3 of total delay without
			// any checking.
			long initialDelay = maxWaitTimeInMilliSec / 3;
			LOGGER.info("Waiting for :" + initialDelay + "seconds, initial wait time");
			LOGGER.info("Waiting for :" + (initialDelay / (1000 * 60)) + "minutes, initial wait time");
			tapEnv.waitTill(initialDelay);

			// 2. Second Delay - For some time and verify if device is accessible with
			// existing ip
